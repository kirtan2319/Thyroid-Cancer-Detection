...
class BaseNet(nn.Module):
    ...
    def train(self, epoch):
        ...
        if val_flag:
            ...
            model_path = os.path.join(self.save_path, self.hash + '_' + str(self.seed) + '_best.pth') if self.hyper_search else os.path.join(self.save_path, str(self.seed) + '_best.pth')
            basics.save_state_dict(self.state_dict(), model_path)
            print(f'Saving best model in epoch {epoch} at {model_path}')
            self.patience = 0
    ...
    def test(self):
        ...
        if not self.cross_testing:
            model_path = os.path.join(self.resume_path, f"{self.hash}_{self.seed}_best.pth") if self.hyper_search else os.path.join(self.resume_path, f"{self.seed}_best.pth")
            state_dict = torch.load(model_path)
            print(f'Testing, loaded model from {model_path}')
        else:
            state_dict = torch.load(self.load_path)
            print(f'Testing, loaded model from {self.load_path}')
    ...
    def record_val(self):
        ...
        self.best_log_dict = basics.add_dict_prefix(self.best_log_dict, 'Val ')
        self.wandb.log(self.best_log_dict)
